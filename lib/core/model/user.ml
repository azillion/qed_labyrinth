...
module type S = sig
  ...
  val register : username:string -> password:string -> email:string -> (t, error) result Lwt.t
  val authenticate : username:string -> password:string -> (t, error) result Lwt.t
  val find_by_id : string -> (t option, error) result Lwt.t
  val find_by_username : string -> (t option, error) result Lwt.t
end

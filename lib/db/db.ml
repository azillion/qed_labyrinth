...
module T = struct
  ...
  let user_info = 
    ...
    custom
      ~encode
      ~decode
      (t2 string string)  (* Changed from tup2 to t2 *)
end
...
module Q = struct
  ...
  let add_user =
    Caqti_request.Infix.(
      T.user_info ->! Caqti_type.unit)
      "INSERT INTO users (username, password_hash) VALUES (?, ?)"

  let get_user =
    Caqti_request.Infix.(
      T.user_info ->? T.user_info)
      "SELECT username, password_hash FROM users WHERE username = ?"
end

(* Handle DB errors *)
let or_error m = m |> Result.map_error ~f:(fun e -> Error.of_string (Caqti_error.show e))

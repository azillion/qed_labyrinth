...
module ArchetypeStorage = MakeComponentStorage(ArchetypeComponent)
module PhysicalityStorage = MakeComponentStorage(PhysicalityComponent)
module BehaviorStorage = MakeComponentStorage(BehaviorComponent)
module GoalStorage = MakeComponentStorage(GoalComponent)
...
module World = struct
  ...
  let init redis_conn =
    ...
    | Ok _entity_ids ->
      ...
      let* () = ArchetypeStorage.load_from_db () in
      let* () = PhysicalityStorage.load_from_db () in
      let* () = BehaviorStorage.load_from_db () in
      let* () = GoalStorage.load_from_db () in
  ...
  let sync_to_db () =
    let db_operation (module Db : Caqti_lwt.CONNECTION) =
      ...
      let* () = ArchetypeStorage.sync_to_db (module Db) in
      let* () = PhysicalityStorage.sync_to_db (module Db) in
      let* () = BehaviorStorage.sync_to_db (module Db) in
      let* () = GoalStorage.sync_to_db (module Db) in

module Handler : Client_handler.S = struct
  ...
  let send_success (client : Client.t) message =
    let%lwt msg = Communication.create 
    ...
    | Ok msg ->
        client.send (Protocol.CommandSuccess {
          message = Types.chat_message_of_model msg
        })
    | Error _ -> Lwt.return_unit

  let world_gen_climate_to_area_climate (climate : World_gen.WorldGen.climate) : Area.climate =
    {
  ...
  let get_room_name (climate : World_gen.WorldGen.climate) =
    match Area.get_room_type { 
      id = ""; name = ""; description = ""; x = 0; y = 0; z = 0;
  ...
  let handle_world_generation (state : State.t) (client : Client.t) =
    ...
    | Authenticated { user_id; character_id = Some character_id } -> 
        ...
        | Ok user -> 
            ...
            | Player | Admin -> 
                let%lwt () = send_error client "You are not authorized to generate the world" in
                Lwt.return_unit
            | SuperAdmin -> 
                ...
                | Ok _ ->
                    ...
                    let%lwt _result = Area.delete_all_except_starting_area "00000000-0000-0000-0000-000000000000" in
                    ...
                    let coord_map = Hashtbl.create 1000 in
                    
                    (* Generate and create areas *)
                    let rec create_areas x y z =
                      if z >= params.depth then Lwt.return_unit
                      else if y >= params.height then create_areas x 0 (z + 1)
                      else if x >= params.width then create_areas 0 (y + 1) z
                      else if x = 0 && y = 0 && z = 0 then 
                        (* Skip starting area coordinates *)
                        create_areas (x + 1) y z
                      else
                        let climate = World_gen.WorldGen.generate_climate params noise_gens (x, y, z) in
                        let name = get_room_name climate in
                        let%lwt area_result = Area.create_with_climate
                          ~name
                          ~description:(Area.get_climate_description {
                          ...
                          ~x ~y ~z
                          ~climate:(world_gen_climate_to_area_climate climate)
                          ()
                        in
                    ...
                    let%lwt () = Hashtbl.fold (fun (x, y, z) area_id acc ->
                      let directions = [
                        (Area.North, (0, 0, 1)); 
                        (Area.South, (0, 0, -1));
                        (Area.East, (1, 0, 0));
                      ...
                      let%lwt () = acc in
                      Lwt_list.iter_s (fun (dir, (dx, dy, dz)) ->
                        let tx = x + dx in
                        let ty = y + dy in
                        let tz = z + dz in
                        match Hashtbl.find_opt coord_map (tx, ty, tz) with
                        | Some target_id ->
                            let%lwt _ = Area.create_exit 
                              ~from_area_id:area_id ~to_area_id:target_id 
                              ~direction:dir ~description:None ~hidden:false ~locked:false in
                            let%lwt _ = Area.create_exit 
                              ~from_area_id:target_id ~to_area_id:area_id 
                              ~direction:(opposite_direction dir) ~description:None ~hidden:false ~locked:false in
                            Lwt.return_unit
                        | None -> Lwt.return_unit
                      ) directions
                    ) coord_map Lwt.return_unit in
                    
                    (* Notify client and broadcast update *)
                    let%lwt () = send_success client "World generation completed successfully" in

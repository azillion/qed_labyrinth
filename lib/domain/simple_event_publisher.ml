...
(* Simple translator from internal events to outbound protobuf messages *)
let process_event (state : State.t) (_trace_id : string option) (event : Event.t)
  ...
  | Event.SendInventory { user_id; items } ->
      let pb_items =
        List.map items ~f:(fun (item_id, name_, description_, quantity_) ->
            Schemas_generated.Output.{
              id = item_id;
              name = name_;
              description = description_;
              quantity = Int32.of_int_exn quantity_;
            })
      in
